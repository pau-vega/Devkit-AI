...
This is a **Claude Code plugin marketplace** containing three plugins: `typescript-rules`, `jsdoc-standards`, and `workflow-toolkit`. It ships an `npx devkit-ai` installer that copies the same three plugins into Claude Code, Cursor, or OpenCode, and the marketplace is being extended (per `.planning/STATE.md`, Phase 1 done, Phase 2 in progress) so all three runtimes get the same skill / command / hook surface.
...
- `package.json` + `bin/install.mjs` + `src/installer/*.mjs` — `devkit-ai` npm package, an `@clack/prompts`-based installer that copies the three plugins into Claude Code, Cursor, or OpenCode at any of three scopes
