...
This is a **Claude Code plugin marketplace** containing two plugins: `typescript-rules` and `jsdoc-standards`. It ships an `npx devkit-ai` installer that copies both plugins into Claude Code, Cursor, or OpenCode, and the marketplace is being extended (per `.planning/STATE.md`, Phase 1 done, Phase 2 in progress) so all three runtimes get the same skill / command / hook surface.

The repo is mostly markdown, JSON configs, shell scripts, and a static HTML landing page — but there is now also a Node-based installer (`bin/install.mjs` + `src/installer/*.mjs`, ESM, one runtime dep: `@clack/prompts`). The build step for the landing page is still a single bash script.
...
- `package.json` + `bin/install.mjs` + `src/installer/*.mjs` — `devkit-ai` npm package, an `@clack/prompts`-based installer that copies the plugins into Claude Code, Cursor, or OpenCode at any of three scopes
...
The marketplace is a *collection* of plugins, not a single plugin — there is no `ai-devkit` plugin to load directly. Two options:
...
There are no lint or test commands. Validation is done by loading the plugin in Claude Code and exercising the hooks, command, and agent, or by running the installer against a scratch directory.
...
The installer (`bin/install.mjs`) layers a fourth axis on top: per-runtime target-root resolution in `src/installer/targets.mjs` (Claude Code, Cursor, OpenCode × project, project-local, user), `@clack/prompts`-driven flow in `src/installer/prompts.mjs`, per-file conflict handling in `src/installer/conflicts.mjs`, and `src/installer/gitignore.mjs` for the project-local delimited block in `.gitignore`. OpenCode hooks and Cursor agents are documented skip rules — see the README's "Known limitations" section.
...
- Installer source files are ESM `.mjs` and import with explicit `.mjs` extensions; the package has `"type": "module"`. The sole runtime dep is `@clack/prompts` — do not add others without a reason
- After editing `marketplace.json` or any plugin component, run `scripts/build-marketplace.sh` to refresh `marketplace.html`
- The top-level `version` field and every plugin's `version` field in `.claude-plugin/marketplace.json` are bumped automatically by release-please on every release, in lockstep with `package.json`. See the `extra-files` config in `.github/release-please-config.json`
...
**Current state:** Phase 1 (Skills Compatibility, OpenCode + Cursor) complete; Phase 2 (Commands and Agents Port) pending. Run `/gsd-plan-phase 2` to begin.
