...
must_haves:
  truths:
    - "User can run `npx @pau-vega/my-marketplace` (after .npmrc auth) and reach an interactive prompt"
    - "User can run `npx github:pau-vega/my-marketplace` with zero auth and reach the same prompt"
  ...
  artifacts:
    - path: "package.json"
      ...
      contains: "@pau-vega/my-marketplace"
...
Output: A runnable installer (`npx @pau-vega/my-marketplace` and `npx github:pau-vega/my-marketplace`), a publish workflow, and README docs. No edits to the existing plugin sources.
</objective>
...
<task type="auto">
  ...
  <action>
Create the npm package shell that makes `npx @pau-vega/my-marketplace` and `npx github:pau-vega/my-marketplace` both work.

1. Write `package.json` at repo root with exactly these fields (per RESEARCH.md §1, §5):
   - `"name": "@pau-vega/my-marketplace"` (scoped — required for GitHub Packages)
   - `"version": "1.0.0"`
   - `"description": "Install the my-marketplace plugins (typescript-rules, jsdoc-standards, workflow-toolkit) into Claude Code, Cursor, or OpenCode."`
   - `"type": "module"`
   - `"bin": { "my-marketplace": "bin/install.mjs" }`
   ...
   - `"repository": { "type": "git", "url": "git+https://github.com/pau-vega/my-marketplace.git" }`
...
6. **`src/installer/gitignore.mjs`** — idempotent block manager. Export:
   - `upsertGitignoreBlock({ cwd, entries })` where `entries` is `string[]` of repo-relative paths. Read `<cwd>/.gitignore` (create if missing). If a block delimited by `# >>> my-marketplace` and `# <<< my-marketplace` exists, replace its contents; else append. Trailing newline preserved. No duplicate entries within the block.
...
7. **`src/installer/prompts.mjs`** — the `@clack/prompts` flow. Export:
   ...
   - Order: `intro("my-marketplace installer")` -> `select` editor -> `select` scope -> `multiselect` plugins (initialValues = all 3 names from `listPlugins`, `required: true`) -> after each, `if (isCancel(v)) { cancel("Cancelled."); process.exit(0); }`.
...
- Fetch anything over the network at runtime.
  ...
  <verify>
    <automated>
      ...
      grep -q ">>> my-marketplace" src/installer/gitignore.mjs && \
      grep -q "<<< my-marketplace" src/installer/gitignore.mjs && \
...
1. **Install (one-liner with auth)** — explains the `~/.npmrc` setup with classic PAT (`read:packages` scope only) and then `npx @pau-vega/my-marketplace`. Quote the two-line `.npmrc` block exactly:
   ```
   @pau-vega:registry=https://npm.pkg.github.com
   //npm.pkg.github.com/:_authToken=<your-classic-pat>
   ```

2. **Install (zero-auth fallback)** — `npx github:pau-vega/my-marketplace`. State the trade-off: clones HEAD, no version pinning, slightly slower first run.
...
- If `README.md` does not yet exist, create it with the installer section as the entire content; do not invent unrelated badges or table-of-contents.
  ...
  <verify>
    <automated>
      test -f README.md && \
      grep -q "npx @pau-vega/my-marketplace" README.md && \
      grep -q "npx github:pau-vega/my-marketplace" README.md && \
...
2. Dry-run against a temp project directory (no writes should happen):
   ...
   node ~/Documents/my-marketplace/bin/install.mjs --dry-run
   ```
   Expected:
...
3. Real run, project scope:
   ```
   cd /tmp/mm-smoke && node ~/Documents/my-marketplace/bin/install.mjs
   ```
...
4. Re-run for conflict path (same directory):
   ```
   node ~/Documents/my-marketplace/bin/install.mjs
   ```
...
6. Project-local scope check:
   ...
   - Verify `/tmp/mm-smoke/.gitignore` has a `# >>> my-marketplace` ... `# <<< my-marketplace` block listing every installed file.
...
- `npx github:pau-vega/my-marketplace` (after pushing) runs the installer for a user with no auth setup. (Verified manually post-merge — not in this plan's verify chain since it requires a push.)
