...
### Locked Decisions
- Distribution: GitHub Packages npm registry, scope `@pau-vega/Devkit-AI`. Invocation: `npx @pau-vega/Devkit-AI`. README also documents zero-config fallback `npx github:pau-vega/Devkit-AI`.
...
The minimum viable shape for `npx @pau-vega/Devkit-AI`:

**`package.json` (root):**
```json
{
  "name": "@pau-vega/Devkit-AI",
  "version": "1.0.0",
  "type": "module",
  "bin": {
    "Devkit-AI": "bin/install.mjs"
  },
...
**Why this works under `npx`:** When a user runs `npx @pau-vega/Devkit-AI`, npm caches the package in `~/.npm/_npx/<hash>/node_modules/@pau-vega/Devkit-AI/`, then invokes the `bin` entry. The script's `import.meta.url` resolves inside that cache directory, so `path.resolve(__dirname, "..")` lands on the package root — which is where the bundled `typescript-rules/`, `jsdoc-standards/`, `workflow-toolkit/` directories live (because `files` shipped them).
...
**Zero-auth fallback that works today:** `npx github:pau-vega/Devkit-AI`. npm clones the repo at the default branch and runs `bin`. The trade-off is no version pinning and a slightly slower first run — fine for a "just try it" path. Document both.
...
**Mitigation:** README documents both the `.npmrc` setup AND the `npx github:pau-vega/Devkit-AI` zero-auth fallback. The two-step path is the headline; the fallback is the escape hatch.
...
Recommendation: option 2. The installer maintains a `.gitignore` block delimited by markers (`# >>> Devkit-AI ... # <<< Devkit-AI`) so re-running it is idempotent. Same approach for Cursor and OpenCode project-local.

### P9. Idempotency on re-run
If a user runs `npx @pau-vega/Devkit-AI` twice with the same answers, the second run should be a no-op (or all conflicts resolve via "skip"). The per-conflict prompt already handles this — but the "remember choice" toggle means a single keystroke in the second run skips everything. Test that path explicitly during plan execution.
